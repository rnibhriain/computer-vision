...
void SupportVectorMachineDemo(Mat& class1_samples, char* class1_name, Mat& class2_samples, char* class2_name, Mat& unknown_samples)
{
	...
	for (int current_class = 1; current_class<=UNKNOWN_CLASS; current_class++)
	{
		...
		if (current_class == 1)
			cvtColor(class1_samples, gray_image, COLOR_BGR2GRAY);
		else if (current_class == 2)
			cvtColor(class2_samples, gray_image, COLOR_BGR2GRAY);
		else cvtColor(unknown_samples, gray_image, COLOR_BGR2GRAY);
...
#define NUMBER_OF_IMAGES_PER_FACE 3
	...
	for (; face_number<=NUMBER_OF_FACES; face_number++)
	{
		for (int image_number = 1; image_number<=NUMBER_OF_IMAGES_PER_FACE; image_number++)
		{
			...
			cvtColor(current_image, current_image, COLOR_GRAY2BGR);
			if (image_number == 2)
			{
				if (face_number%10 == 1)
				{
	...
	for (face_number = 0; face_number < (int)unknown_face_images.size(); face_number++)
	{
		...
		cvtColor(unknown_face_images[face_number], temp_image1, COLOR_GRAY2BGR);
...
void PCASimpleExample()
{
	...
	{  // OPTIONAL: Write covariance matrices on images
		...
		calcCovarMatrix(samples_matrix,covariance,average, cv::COVAR_NORMAL | cv::COVAR_ROWS);
		calcCovarMatrix(transformed_samples_matrix,new_covariance,new_average, cv::COVAR_NORMAL | cv::COVAR_ROWS);
...
//	cv::destroyAllWindows();
	...
	cvtColor(numbers, gray_numbers, COLOR_BGR2GRAY);
	threshold(gray_numbers,binary_numbers,128,255,THRESH_BINARY_INV);
    vector<vector<Point>> contours;
	vector<Vec4i> hierarchy;
	findContours(binary_numbers,contours,hierarchy,cv::RETR_TREE,cv::CHAIN_APPROX_NONE);
	...
	cvtColor(template1locations, template1locations_gray, COLOR_BGR2GRAY);
	...
	matchTemplate( full_image, template2, correlation_image, cv::TM_CCORR_NORMED );
	minMaxLoc( correlation_image, &min_correlation, &max_correlation );
	FindLocalMaxima( correlation_image, matched_template_map, max_correlation*0.99 );
	timer->recordTime("Template Matching (2)");
	Mat matched_template_display2;
	cvtColor(matched_template_map, matched_template_display2, COLOR_GRAY2BGR);
	...
	cvtColor(template2locations, template2locations_gray, COLOR_BGR2GRAY);
	...
	cvtColor(correlation_window1, correlation_display1, COLOR_GRAY2BGR);
	cvtColor(correlation_window2, correlation_display2, COLOR_GRAY2BGR);
	...
	Mat current_frame;
	// Chamfer matching
	if (bicycle_video.isOpened())
	{
		Mat model_gray, model_edges, model_edges2;
		cvtColor(bicycle_model, model_gray, COLOR_BGR2GRAY);
		threshold(model_gray, model_edges, 127, 255, THRESH_BINARY);
		bicycle_video.set(cv::CAP_PROP_POS_FRAMES, 400);  // Just in case the video has already been used.
		bicycle_video >> current_frame;
		bicycle_background = current_frame.clone();
		bicycle_video.set(cv::CAP_PROP_POS_FRAMES, 500);
		timer->reset();
		int count = 0;
		while (!current_frame.empty() && (count < 8))
		{
			...
			cvtColor(difference_frame, difference_gray, COLOR_BGR2GRAY);
			...
			distanceTransform(current_edges, chamfer_image, cv::DIST_L2, 3);
			...
			cvtColor(current_edges, current_edges_display, COLOR_GRAY2BGR);
			cvtColor(local_minima, local_minima_display, COLOR_GRAY2BGR);
			cvtColor(model_edges, model_edges_display, COLOR_GRAY2BGR);
			cvtColor(matching_display_image, colour_matching_display_image, COLOR_GRAY2BGR);
			cvtColor(chamfer_display_image, colour_chamfer_display_image, COLOR_GRAY2BGR);
	...
	if (people_video.isOpened())
	{
		HOGDescriptor hog;
		hog.setSVMDetector(HOGDescriptor::getDefaultPeopleDetector());
		int frame_number = 990;
		people_video.set(cv::CAP_PROP_POS_FRAMES, frame_number);
