...
void ShowHLS(Mat image)
{
	...
	cvtColor(hls_planes[0], channel1_display, COLOR_GRAY2BGR);
	cvtColor(hls_planes[1], channel2_display, COLOR_GRAY2BGR);
	cvtColor(hls_planes[2], channel3_display, COLOR_GRAY2BGR);
	output1 = JoinImagesHorizontally(image, "Original", channel1_display, "Hue");
	output2 = JoinImagesHorizontally(channel3_display, "Saturation", channel2_display, "Luminance", 4);
	output3 = JoinImagesVertically(output1, "", output2, "", 4);
	imshow("HLS components", output3);
}
...
void writeText(Mat image, char* text, int row, int column, Scalar passed_colour, double scale, int thickness)
{
	...
	writeText(image, text, row, column, passed_colour, scale, thickness);
}
...
Mat JoinImagesHorizontally(Mat& image1, const char* name1, Mat& image2, const char* name2, int spacing, Scalar passed_colour/*=-1.0*/)
{
	string name1_str(name1), name2_str(name2);
	return JoinImagesHorizontally(image1, name1_str, image2, name2_str, spacing, passed_colour);
}
...
Mat JoinImagesVertically(Mat& image1, const char* name1, Mat& image2, const char* name2, int spacing, Scalar passed_colour/*=-1.0*/)
{
	string name1_str(name1), name2_str(name2);
	return JoinImagesVertically(image1, name1_str, image2, name2_str, spacing, passed_colour);
}

Mat JoinImagesVertically( Mat& image1, string name1, Mat& image2, string name2, int spacing, Scalar passed_colour/*=-1.0*/ )
{
...
{
	int codec = static_cast<int>(video_to_emulate.get(cv::CAP_PROP_FOURCC));
	Size image_size = Size((int) video_to_emulate.get(cv::CAP_PROP_FRAME_WIDTH),
                           (int) video_to_emulate.get(cv::CAP_PROP_FRAME_HEIGHT));
	double fps = video_to_emulate.get(cv::CAP_PROP_FPS);
...
Mat ComputeDefaultImage( Mat& passed_image )
{
	...
	cvtColor(opened_image, image_gray, COLOR_BGR2GRAY);
	Canny(image_gray,image_gray_edges,100,150);
	cvtColor(image_gray_edges, image_edges, COLOR_GRAY2BGR);
	...
	// Code for the following function is from https://stackoverflow.com/questions/33041900/opencv-threshold-with-mask/33069436#33069436
	...
	{
		double x_difference = point2.x - point1.x;
		double y_difference = point2.y - point1.y;
